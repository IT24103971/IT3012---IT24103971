...
class SearchAgent:
    ...
    def sense_and_act(self, percept: dict) -> str:
        ...
        if not self.plan:
            ...
            if self.active_algo == 'BFS':
                ...
                path = self.astar_search(start_pos, closest_food, walls, grid_size, tile_percepts=tile_percepts)
            else:
                path = self.bfs_search(start_pos, closest_food, walls, grid_size)
    ...
    def astar_search(self, start_pos, goal_pos, walls, grid_size=None, heuristic_type='manhattan', tile_percepts=None):
        start = tuple(start_pos)
        goal = tuple(goal_pos)
        walls_set = {tuple(w) for w in walls}
        ...
        while pq:
            ...
            for action, (dx, dy) in directions:
                ...
                # Validate Feasibility using Knowledge Base
                if tile_percepts and next_pos in tile_percepts:
                    self.kb.clear_facts()
                    for fact in tile_percepts[next_pos]:
                        self.kb.tell_fact(fact)
                    self.kb.forward_chain()
                    if 'Retreat' in self.kb.facts:
                        # Marked as Infeasible, skip node
                        continue
